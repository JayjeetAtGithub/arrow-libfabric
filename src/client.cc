...
int main(int argc, char** argv)
{
    ...
    // Read the protocol from the CLI
    char* protocol = argv[1];

    // Read the server address from the CLI
    char* server_address = argv[2];

    // Initialize the Mercury client and don't listen for incoming RPCs
    client_data.hg_class = HG_Init(protocol, HG_FALSE);
    assert(client_data.hg_class != NULL);

    // Create a Mercury context
    ...
    // Destroy the Mercury context
    ret = HG_Context_destroy(client_data.hg_context);
    assert(ret == HG_SUCCESS);

    // Release resources allocated by Mercury
